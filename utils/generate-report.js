...
//
// TC metadata (steps, expected result, test data) is loaded from
//   test-data/{feature}Metadata.js  when present.
// Without metadata the report still generates but uses generic placeholders.
//
...
// ── Load TC metadata (steps / expected result / test data per TC ID) ──────────
// File: test-data/{featureLower}Metadata.js  — optional, falls back to placeholders
const metadataPath = path.join(__dirname, `../test-data/${feature.toLowerCase()}Metadata.js`);
const tcMetadata   = fs.existsSync(metadataPath) ? require(metadataPath) : {};
if (Object.keys(tcMetadata).length > 0) {
  console.log(`📋 Loaded TC metadata: ${metadataPath} (${Object.keys(tcMetadata).length} TCs)`);
} else {
  console.warn(`⚠  No metadata file found at ${metadataPath} — using placeholder steps/expected`);
}
...
const TC_HEADER = [
  ...
  'Layer','Preconditions','Test Steps','Test Data',
...
const DAILY_HEADER = [
  ...
  'Layer','Priority','Status','Bug ID','Bug Title','Severity','Bug Priority',
...
// Strip leading "TC-XXX-NNN | " prefix from Playwright test titles
function cleanTitle(title) {
  return title.replace(/^TC-[\w-]+\s*\|\s*/, '').trim();
}
...
const tcRows = tests.map((t, i) => {
  ...
  const meta   = tcMetadata[tcId] || {};                  // ← metadata lookup
  ...
  return {
    'TC ID':           tcId,
    'Test Case Name':  cleanTitle(t.title),               // ← strip TC-ID prefix
    'Module':          feature,
    'Test Type':       getTestType(t.title),
    'Priority':        sev,
    'Layer':           layer,
    'Preconditions':   meta.preconditions ||
                         'Application accessible at http://customerportal.dev-ts.online; user sajith_xyz active',
    'Test Steps':      meta.steps         ||
                         `(See Playwright spec: tests/${feature.toLowerCase()}.spec.js)`,
    'Test Data':       meta.testData      ||
                         `See test-data/${feature.toLowerCase()}Data.js`,
    'Expected Result': meta.expectedResult ||
                         'Test assertions pass per spec definition',
    'Actual Result':   status === 'PASS'
                         ? 'All assertions passed via Playwright'
                         : (t.error || 'Test failed'),
    'Status':          status,
    'Executed By':     'Playwright Automation',
...
setColWidths(tcSheet, [14,46,10,12,10,8,32,55,42,50,50,9,22,14,42,28]);
...
const bugRows = failedTests.map((t, i) => {
  ...
  if (isRegression) {
    ...
    if (existing) return {
      ...
      'Remarks': `Still failing in Playwright — ${now}`,
    };
  }

  return {
    'Bug ID':              bugId,
    'Title':               cleanTitle(t.title),           // ← strip TC-ID prefix
    'Module':              feature,
    'Linked TC ID':        tcId,
    'Severity':            sev,
    'Priority':            getPriority(sev),
    'Environment':         'http://customerportal.dev-ts.online | Chrome | Windows | Headless',
    'Steps to Reproduce':  meta.bugSteps || meta.steps  ||
                             `1. Navigate to ${feature} feature\n2. Execute: ${tcId}\n3. Observe failure`,
    'Expected Result':     meta.expectedResult           ||
                             'All Playwright assertions should pass',
...
if (isRegression) {
  ...
  existingBugRows.forEach(bug => {
    if (passedIds.includes(bug['Linked TC ID'])) {
      ...
      bug['Remarks']   = `Fixed — verified in Playwright regression ${now}`;
    }
  });
}
...
setColWidths(bugSheet, [18,50,10,14,10,8,42,52,42,52,14,22,14,12,16,30]);
...
if (failedTests.length === 0) {
  ...
  if (!existingTcIds.has(summaryKey)) {
    newDailyRows.push({
      ...
      'Test Case Name': `All ${tests.length} TCs passed — ${feature} module`,
      'Test Type': 'All types', 'Layer': 'Unit / API / Auto', 'Priority': 'Critical–Low',
      'Status': 'PASS', 'Bug ID': '—',
      'Bug Title': `No bugs — ${tests.length}/${tests.length} TCs passed`,
  ...
  failedTests.forEach((t, i) => {
    ...
    const sev   = getSeverity(t.title);
    const meta  = tcMetadata[tcId] || {};
    const bugId = `BUG-${feature.toUpperCase()}-${String(dailyBugNum++).padStart(3,'0')}`;
    newDailyRows.push({
      'Date': today, 'Feature': feature, 'TC ID': tcId,
      'Test Case Name': cleanTitle(t.title),
      'Test Type': getTestType(t.title), 'Layer': getLayer(t.title), 'Priority': sev,
      'Status': 'FAIL', 'Bug ID': bugId,
      'Bug Title': cleanTitle(t.title),
      'Severity': sev, 'Bug Priority': getPriority(sev),
      'Environment': 'http://customerportal.dev-ts.online',
      'Browser': 'Chrome', 'OS': 'Windows',
      'Steps to Reproduce': meta.bugSteps || meta.steps ||
        `1. Navigate to ${feature} feature\n2. Execute: ${tcId}\n3. Observe failure`,
      'Expected Result': meta.expectedResult || 'All Playwright assertions should pass',
      'Actual Result': t.error || 'Playwright test failed',
      'Source Report': `Test Execution Report\\Feature Reports\\${feature}.xlsx`,
    });
  });
...
if (dwb.SheetNames.includes('Daily Bug Report')) {
  dwb.SheetNames.splice(dwb.SheetNames.indexOf('Daily Bug Report'), 1);
  delete dwb.Sheets['Daily Bug Report'];
}
