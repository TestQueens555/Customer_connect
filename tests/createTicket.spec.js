...
// ── Login helper — matches ActionQueue pattern for CI stability ───────────────
async function loginAndNavigate(page) {
  ...
  const tp = new CreateTicketPage(page);
...
test('TC-CT-001 | Submit valid ticket with all required fields', async ({ page }) => {
  const tp = await loginAndNavigate(page);
  ...
  if (t === 'Submit Ticket?') { await tp.dismissSwal(); await page.waitForTimeout(1000); }
  await expect(tp.swalPopup).toBeVisible({ timeout: 8000 });
